...
def test_stock_manager_room_filters_and_delivery(
    ...
    with sync_playwright() as playwright:
        ...
        page.get_by_label(re.compile(r"^Model")).fill("Corrected firewall model")
        page.evaluate("window.scrollTo(0, 0)")
        page.screenshot(path=str(screenshots / "workspace-edit-asset.png"), full_page=True)
        page.get_by_role("button", name="Save asset", exact=True).click()
        expect(page.locator(".definition-list")).to_contain_text("Reception firewall")
        expect(page.locator(".definition-list")).to_contain_text("Corrected firewall model")
        ...
        page.get_by_role("link", name="Put selected in use").click()
        expect(page.locator("#asset-picker-selection-count")).to_contain_text("1")
        page.locator('[name="notes"]').fill("Installed in server room X")
        page.screenshot(path=str(screenshots / "workspace-put-in-use.png"), full_page=True)
        page.get_by_role("button", name="Put in use", exact=True).click()
        page.wait_for_url("**/transactions/*/")
        page.goto(url + "?status=in_use")
        expect(rows).to_have_count(1)
        rows.locator('input[type="checkbox"]').check()
        page.get_by_role("link", name="Return from internal use", exact=True).click()
        expect(page.locator("#asset-picker-selection-count")).to_contain_text("1")
        page.locator('[name="location"]').select_option(str(location_tree["room"].pk))
        page.locator('[name="notes"]').fill("Removed from server room X")
        page.get_by_role("button", name="Return from internal use", exact=True).click()
        page.wait_for_url("**/transactions/*/")
        page.goto(url + "?in_storage=1")
        expect(rows).to_have_count(1)
        rows.locator('input[type="checkbox"]').check()
